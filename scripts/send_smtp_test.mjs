...
async function main() {
  ...
  // On Windows the pathname begins with a leading slash, adjust
  const normalized = envPath.replace(/^\/(.:\/)/, '$1');
  const finalPath = normalized;
  if (!fs.existsSync(finalPath)) {
    console.error('.env.local not found at', finalPath);
    process.exit(1);
  }
  const env = parseEnvFile(finalPath);
